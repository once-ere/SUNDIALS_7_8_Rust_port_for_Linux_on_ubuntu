...
re-ran the same gate under the pure-Rust libm on **four hosts** — the same
...
Same tally, and the *same 34 variants name for name*: the four DIFF lists are
byte-identical files, all hashing to `6581e4918e5ab2c71ee6354f383a0f34`. That
spans glibc 2.36 to 2.44 and two rustc versions, so the result is
toolchain-stable as well as distribution-stable.
...
(below), on all four hosts, and they include the three that used to break only
on Arch. The port
