...
re-ran the same gate under the pure-Rust libm on **six hosts** — every
...
Same tally, and the *same 34 variants name for name*: the six DIFF lists are
byte-identical files, all hashing to `6581e4918e5ab2c71ee6354f383a0f34`. That
spans glibc 2.36, 2.40, 2.41, 2.43, 2.44 and musl 1.2.5, over two rustc
versions — so the result is toolchain-stable and **libc**-stable, not merely
distribution-stable.
...
(below), on all six hosts, and they include the three that used to break only
on Arch. The port
