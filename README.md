...
re-ran the same gate under the pure-Rust libm on two hosts:
...
Same tally, and the *same 34 variants name for name*, across two glibc
versions and two rustc versions. The score no longer moves with the host.
...
(below), and they include the three that used to break only on Arch. The port
