...
re-ran the same gate under the pure-Rust libm on **five hosts** — the same
distributions whose libm differences exposed the problem in the first place,
plus the one that was excluded because of them:
...
Same tally, and the *same 34 variants name for name*: the five DIFF lists are
byte-identical files, all hashing to `6581e4918e5ab2c71ee6354f383a0f34`. That
spans glibc 2.36 to 2.44, musl 1.2.5, and two rustc versions — so the result
is toolchain-stable and **libc**-stable, not merely distribution-stable.
...
**not** repeated on musl. And everything here is x86-64; arm64 is untested.
...
(below), on all five hosts, and they include the three that used to break only
on Arch. The port
