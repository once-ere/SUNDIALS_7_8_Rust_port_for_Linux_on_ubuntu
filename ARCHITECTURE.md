...
> **Inherited document.** This describes the translation itself — the handle
> model, the locked porting patterns and the numbered deviation classes —
> which is platform-independent and unchanged in this repository. It predates
> the pure-Rust libm: wherever it says an elementary function comes from the
> host C library, see [`LIBM.md`](LIBM.md), which supersedes that.
