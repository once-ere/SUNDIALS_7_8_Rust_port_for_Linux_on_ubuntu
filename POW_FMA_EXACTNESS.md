...
> **Inherited document.** How far the deterministic `pow` is bit-exact and
> how that was established. Still current: `pow` is unchanged here, and
> `tools/pow_differential.sh` re-measured it on this machine against glibc
> 2.43 — **0 mismatches over 5,900,000 domain inputs and 0 over 20,000,000
> unrestricted finite inputs** (`logs/pow_differential.log`). The same
> methodology is now applied to the other twelve functions in
> [`LIBM.md`](LIBM.md).
