...
> **Inherited document, superseded.** This is the predecessor repository's
> per-variant matrix against the *shipped upstream* `.out` reference files,
> measured on Ubuntu 24.04 / glibc 2.39. It is kept for its root-cause
> analyses, which remain valid explanations of why individual references are
> stale.
>
> **It is not the verification of this repository.** The measurement here
> compares against *freshly built pristine C binaries on this machine*
> rather than against shipped reference files, and lives in
> [`differences/`](differences/) — 171 of 179 variants byte-identical, and
> 179 of 179 under `--features host-libm`, i.e. 0 port defects. See
